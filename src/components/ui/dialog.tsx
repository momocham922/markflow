    ...
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          ...
          // viewport (e.g. the paywall on a phone) is clipped top AND bottom by
          // the translate-y centering and cannot scroll. --dialog-max-h is
          // 100dvh minus a 2rem gutter minus the iOS safe-area insets (notch /
          // home indicator), so the centered box never extends under them; it
          // tracks mobile browser/keyboard chrome via 100dvh. Short dialogs keep
          // their natural height.
          "fixed top-[50%] left-[50%] z-50 grid max-h-[var(--dialog-max-h)] w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 overflow-y-auto overscroll-contain rounded-lg border bg-background p-6 shadow-lg duration-200 outline-none data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 sm:max-w-lg",
          className,
        )}
        {...props}
      >
