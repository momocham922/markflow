...
impl<R: Runtime> Iap<R> {
    ...
    /// Present the store-native "manage subscriptions" UI (iOS StoreKit 2 sheet;
    /// Android opens the Play subscriptions center). No payload/response.
    pub async fn show_manage_subscriptions(&self) -> crate::Result<()> {
        self.0
            .run_mobile_plugin_async("showManageSubscriptions", ())
            .await
            .map_err(Into::into)
    }
}
