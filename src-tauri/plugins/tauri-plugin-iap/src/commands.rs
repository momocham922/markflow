...
/// Present the store-native "manage subscriptions" surface (iOS StoreKit 2
/// `AppStore.showManageSubscriptions(in:)`). Lets a subscriber cancel/change an
/// IAP subscription IN-APP — the only in-app cancel path Apple provides, and the
/// one that also works for TestFlight/sandbox real-Apple-ID testers whose subs do
/// NOT appear under Settings › Subscriptions. Takes no payload.
#[command]
pub async fn show_manage_subscriptions<R: Runtime>(app: AppHandle<R>) -> Result<()> {
    app.iap().show_manage_subscriptions().await
}
